#!/usr/bin/env node
// Launcher: boots the dsh profile that carries the makima-tui bundle.
...
const PROFILE = process.env.MAKIMA_TUI_PROFILE || 'makima-tui'
...
if (probe.error || probe.status !== 0) {
  console.error('makima tui: the `dsh` CLI is not on PATH.')
...
if (!existsSync(join(profileDir, 'package.json'))) {
  console.error(`makima-tui: profile "${PROFILE}" is not set up yet.`)
