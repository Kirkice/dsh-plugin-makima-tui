...
const useTempLog = (): string => {
  ...
  process.env.MAKIMA_TUI_RENDER_DEBUG_FILE = file

  return file
}

afterEach(() => {
  delete process.env.MAKIMA_TUI_RENDER_DEBUG
  delete process.env.MAKIMA_TUI_RENDER_DEBUG_FILE
...
describe('render-debug', () => {
  ...
  it('records the row and the two numbers that explain why it was dropped', () => {
    const file = useTempLog()
    process.env.MAKIMA_TUI_RENDER_DEBUG = '1'
  ...
  it('treats 0 and false as off, so an inherited env var cannot switch it on', () => {
    ...
    for (const v of ['0', 'false', '']) {
      process.env.MAKIMA_TUI_RENDER_DEBUG = v
  ...
  it('swallows a write failure instead of throwing into the render loop', () => {
    process.env.MAKIMA_TUI_RENDER_DEBUG = '1'
    process.env.MAKIMA_TUI_RENDER_DEBUG_FILE = join(tmpdir(), 'no-such-dir-here', 'nested', 'trace.log')
