...
export function handleMouseEvent(app: App, m: ParsedMouse): void {
  ...
  if (m.action === 'press') {
    ...
    // A bare press only arms drag selection; it has no focus and therefore
    // paints no overlay. Avoid rendering for that transient state. Besides
    // doing needless work on every click, the two press/release renders could
    // alternate reusable front/back buffers while the terminal was handling
    // the click, exposing stale or externally disturbed cells. A real drag
    // notifies on its first motion. Replacing an existing visible selection
    // still needs an immediate repaint to remove its overlay.
  ...
  // Drag release changes a visible selection from live to stable and must
  // notify copy-on-select subscribers. A click-release without motion has no
  // visible selection and must not manufacture a second no-op render.
