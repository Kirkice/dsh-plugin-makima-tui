...
class ProbeAdapter extends LlmAdapter {
  async *stream(options) {
    ...
    // `THINK` asks for a long reasoning stream, so the transcript's thinking
    // block can be seen at the length a real turn produces rather than the two
    // words a scripted reply would.
    if (user.includes('THINK')) {
      for (let i = 1; i <= 60; i++) {
        yield { index: 0, text: `reasoning line ${i}: weighing the options and writing enough of them to fill a screen\n`, type: 'reasoning-delta' }
...
const searchProvider = {
  ...
  search: async request => ({
    ...
    sources: [
      ...
      { title: 'makima tui', url: 'https://github.com/agentforce314/makima tui' }
    ],
    truncated: false
  })
}
