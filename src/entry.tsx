...
// (so Termux + MAKIMA_TUI_INLINE=0 is fullscreen too). Only fullscreen should
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `makima-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
if (process.env.MAKIMA_TUI_HEAPDUMP_ON_START === '1') {
