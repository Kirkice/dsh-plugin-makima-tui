...
export function useMainApp(gw: GatewayClient) {
  ...
  // …and its reasoning twin. Deliberately the GLOBAL mode alone, with no
  // section pin: `thinking: 'expanded'` means "let the stream be visible", not
  // "print the whole chain of thought". ToolTrail derives it the same way.
  const thinkingDetailsExpanded = ui.detailsMode === 'expanded'
  ...
  const detailsLayoutKey = useMemo(() => {
    ...
    // Reasoning swaps between a one-or-two-line preview and the whole chain,
    // which is a far bigger height swing than the tool variants — it has to
    // bucket the cache too.
    return `${thinking}:${tools}:${toolsDetailsExpanded ? 'x' : '-'}:${thinkingDetailsExpanded ? 'x' : '-'}`
  }, [thinkingDetailsExpanded, toolsDetailsExpanded, ui.detailsMode, ui.detailsModeCommandOverride, ui.sections])
  ...
  const onMemorySelect = useCallback(
    (path: string) => {
      ...
      void openMemoryFileInEditor(path, {
        cwd: getUiState().info?.cwd || process.env.MAKIMA_TUI_WORKSPACE || process.env.MAKIMA_TUI_CWD || process.cwd(),
  ...
  const cwd = ui.info?.cwd || process.env.MAKIMA_TUI_CWD || process.cwd()
