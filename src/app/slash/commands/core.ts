import { forceRedraw, type MouseTrackingMode } from '@makima-tui/ink'
...
export const DASHBOARD_EXIT_DISABLED_MESSAGE = 'exit is disabled in hosted dashboard chat — use /new to start a fresh session'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update makima tui to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
...

  {
    ...
    run: (arg, ctx) => {
      ...
      if (second && isSectionName(first)) {
        ...
        gateway.rpc<ConfigSetResponse>('config.set', { key: `details_mode.${first}`, value: mode ?? '' }).catch(() => {})
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try MAKIMA_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const next: null | StatusBarMode =
        !mode || mode === 'toggle' ? toggle : mode === 'on' || mode === 'top' ? 'top' : mode === 'off' || mode === 'bottom' ? mode : null
