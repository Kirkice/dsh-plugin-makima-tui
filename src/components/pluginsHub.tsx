...
const roleLabel = (row: PluginRow): string =>
  [row.builtIn ? 'built-in' : '', row.bundle ? 'bundle' : '', row.dependency ? 'dependency' : ''].filter(Boolean).join(', ')
...
export function PluginsHub({ gw, onClose, t }: PluginsHubProps) {
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.text} wrap="wrap">
          Remove {selected.packageName} from profile {profile}?
        </Text>
        <Text color={t.color.muted} wrap="wrap">
          This edits the persisted profile. A DSH restart is required before the plugin is unloaded.
        </Text>
        {choices.map((choice, choiceIdx) => (
          <Text
            bold={confirmIdx === choiceIdx}
            color={choiceIdx === 1 ? t.color.error : t.color.accent}
            inverse={confirmIdx === choiceIdx}
            key={choice}
          >
            {confirmIdx === choiceIdx ? '▸ ' : '  '}
            {choice}
          </Text>
        ))}
  ...
  if (view === 'details' && selected) {
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="wrap">
          specifier: {selected.specifier || '(none)'}
        </Text>
        <Text color={t.color.muted}>roles: {roleLabel(selected) || 'none'}</Text>
        {selected.builtIn ? (
          <Text color={t.color.warn} wrap="wrap">
            Built-in bundles are protected by DSH and cannot be uninstalled.
          </Text>
        ) : (
          <>
            <Text color={t.color.error} inverse>
              ▸ Uninstall plugin
            </Text>
  ...
  const labels = effectiveRows.map(
    (row) => `✓ ${row.packageName}${row.specifier ? ` — ${row.specifier}` : ''}${roleLabel(row) ? ` [${roleLabel(row)}]` : ''}`
  )
  const { items, offset } = windowItems(labels, clampedIdx, VISIBLE)
  const scopeLabel =
    effectiveScope === 'user' ? `${effectiveRows.length} profile dependency plugin(s) · Tab all` : `all ${rows.length} profile plugins`
      ...
      {items.map((row, itemIdx) => {
        ...
        return (
          <Text
            ...
            inverse={active}
            key={effectiveRows[lineIdx]?.packageName ?? row}
            wrap="truncate-end"
          >
            {active ? '▸ ' : '  '}
            {itemIdx + 1}. {row}
