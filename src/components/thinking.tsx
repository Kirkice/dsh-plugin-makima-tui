import { Box, NoSelect, Text } from '@makima-tui/ink'
...
import type { ActiveTool, ActivityItem, DetailsMode, SectionVisibility, SubagentNode, SubagentProgress, ThinkingMode } from '../types.js'
...
export const Thinking = memo(function Thinking({
  ...
  // Whether the preview is standing in for more than it shows. Compared
  // against the same whitespace-collapsed form the preview is cut from, so a
  // reasoning block that fits says nothing and one that does not offers the key
  // that opens it.
  const elided = useMemo(
    () => mode !== 'full' && cleanThinkingText(reasoning).replace(/\s+/g, ' ').trim().length > preview.length,
    [mode, preview, reasoning]
  )
          ...
          mode === 'full' ? (
            lines.map((line, index) => (
              <Text color={t.color.muted} key={index} wrap="wrap-trim">
                {line || ' '}
                {index === lines.length - 1 ? <StreamCursor color={t.color.muted} streaming={streaming} visible={active} /> : null}
              </Text>
            ))
          ) : (
            // Wrapped, not truncate-end: the preview is cut to a length that
            // lands in a line or two at any usual width, and hard-truncating it
            // to exactly one would throw away the half that was kept.
...
export const ToolTrail = memo(function ToolTrail({
  ...
  // Reasoning takes the GLOBAL mode alone, and deliberately not the section
  // pin the way tools do. `thinking: 'expanded'` is what both the built-in
  // default and `/reasoning show` set to mean "let the stream be VISIBLE";
  ...
  const thinkingOverride =
    thinkingKey && detailExpanded && Object.hasOwn(detailExpanded, thinkingKey) ? detailExpanded[thinkingKey] : undefined
  ...
  const resolvedExpandedTools = new Set(
    trail.flatMap((_, index) => {
      ...
      const override = key && detailExpanded && Object.hasOwn(detailExpanded, key) ? detailExpanded[key] : undefined
  ...
  for (const [i, compactLine] of trail.entries()) {
    ...
    const toolOverride = toolKey && detailExpanded && Object.hasOwn(detailExpanded, toolKey) ? detailExpanded[toolKey] : undefined
    ...
    if (parsed) {
      ...
      if (parsed.detail) {
        // The card bounded its own body and said how much it held back. Name
        ...
        pushDetail({
          ...
          content: expandable ? `${parsed.detail} (ctrl+o to expand)` : parsed.detail,
    ...
    panels.push({
      ...
      render: (rails) => (
        <Thinking
          ...
          branch="last"
          mode={thinkingExpanded ? 'full' : 'truncated'}
          rails={rails}
  ...
  const renderGroup = (group: Group, gap: boolean) => {
    ...
    const visualColor =
      visual.tone === 'error' ? t.color.error : visual.tone === 'ok' ? t.color.ok : visual.tone === 'accent' ? t.color.accent : t.color.text
    ...
    return (
      ...
      >
        <Text color={group.color}>
          {group.live ? (
            <Text color={visualColor} dim={!blinkOn}>
              {blinkOn ? `${visual.glyph} ` : '  '}
            </Text>
          ) : (
  ...
  return (
    <Box flexDirection="column">
      {panels.map((panel, index) => (
        <TreeNode branch={index === topCount - 1 ? 'last' : 'mid'} header={panel.header} key={panel.key} open={panel.open} t={t}>
          {panel.render}
        </TreeNode>
      ))}
      {toolsFlat}
