...
const prettyMap = (value: Record<string, string> | undefined) => (value && Object.keys(value).length ? JSON.stringify(value) : '')
...
export function SkillsMcpManagerOverlay({ gw, initialTab, onClose, t }: SkillsMcpManagerOverlayProps) {
    ...
    () =>
      transport === 'stdio'
        ? [
            ...
            ['Arguments (space-separated)', argsOrHeaders],
            ['Working directory', cwdOrEnv],
        ...
        : [
            ...
            ['Transport', transport],
            ['URL', commandOrUrl],
            ['Headers (JSON)', argsOrHeaders]
          ],
    [argsOrHeaders, commandOrUrl, cwdOrEnv, env, name, transport]
  )
  ...
  const inspectSkill = (skill: ManagedSkill) => {
    ...
    setError('')
    void gw
      .request<{ skill?: ManagedSkillDetail }>('skills.manager.read', { path: skill.path })
      .then((result) => {
  ...
  const toggleSkill = (skill: ManagedSkill) => {
    ...
    setError('')
    void gw
      .request('skills.manager.set_enabled', { enabled: !skill.enabled, path: skill.path })
      .then(() => {
  ...
  const toggleServer = (server: McpServer) => {
    ...
    setError('')
    void gw
      .request('mcp.manager.set_enabled', { enabled: server.enabled === false, name: server.name })
      .then(() => {
        const outcome =
          server.enabled === false
            ? server.managed === false
              ? 'enabled by Makima runtime policy'
              : 'enabled and applying'
            : server.managed === false
              ? 'runtime-disabled; its external service is still running'
  ...
  const loadMcpTools = (server: McpServer) => {
    ...
    setError('')
    void gw
      .request<{ tools?: McpTool[] }>('mcp.manager.tools', { name: server.name })
      .then((result) => {
  ...
  const toggleTool = (tool: McpTool) => {
    ...
    setError('')
    void gw
      .request('mcp.manager.set_tool_enabled', { enabled: !tool.enabled, name: tool.name })
      .then(() => {
  ...
  const saveServer = () => {
    ...
    try {
      const server: McpServer =
        transport === 'stdio'
          ? {
              args: argsOrHeaders.split(/\s+/).filter(Boolean),
              command: commandOrUrl,
              cwd: cwdOrEnv,
              enabled: selectedServer?.enabled !== false,
              env: jsonMap(env, 'Environment'),
              name,
              transport
            }
      ...
      setError('')
      void gw
        .request<{ server?: McpServer }>('mcp.manager.save', { server })
        .then(() => {
  ...
  const removeSelected = () => {
    ...
    void request
      .then(() => {
        setNotice(
          tab === 'skills' ? `${selectedSkill?.name ?? 'skill'} deleted` : `${selectedServer?.name ?? 'server'} deleted and unloaded`
        )
  ...
  useInput((ch, key) => {
    ...
    if (field === 1 && (key.leftArrow || key.rightArrow))
      return setTransport((current) => (current === 'stdio' ? 'streamable-http' : 'stdio'))
  ...
  if (loading && view === 'list')
    return <Text color={t.color.muted}>loading {tab === 'skills' ? 'managed skills' : 'MCP configuration'}…</Text>
    ...
    return (
      <Box flexDirection="column" width={width}>
        <Text bold color={t.color.warn}>
          Delete {tab === 'skills' ? 'skill' : 'MCP server'}?
        </Text>
        <Text color={t.color.text}>{label}</Text>
        <Text color={t.color.muted}>
          {tab === 'skills'
            ? 'This removes the managed skill file or bundle.'
            : 'The Harness backend unloads the live connection and unregisters its tools.'}
        </Text>
        <Text bold color={deleteConfirmed ? t.color.error : t.color.accent} inverse={deleteConfirmed}>
          ▸ {deleteConfirmed ? 'Delete permanently' : 'Cancel'}
        </Text>
  ...
  if (view === 'mcp-actions' && selectedServer) {
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>
          {selectedServer.managed === false
            ? 'External service: enable/disable applies Makima runtime policy only.'
            : 'Makima-managed connection: enable/disable starts or stops the client.'}
        </Text>
        <Text color={t.color.muted}>
          {selectedServer.agentIds?.length ? `visible in: ${selectedServer.agentIds.join(', ')}` : 'visible in: global'}
        </Text>
        <Box marginTop={1} flexDirection="column">
          {actions.map((action, row) => (
            <Text key={action} color={row === mcpActionIndex ? t.color.accent : t.color.text}>
              {row === mcpActionIndex ? '▸ ' : '  '}
              {action}
            </Text>
          ))}
  ...
  if (view === 'mcp-tools' && selectedServer) {
    return (
      <Box flexDirection="column" width={width}>
        <Text bold color={t.color.accent}>
          MCP Tools · {selectedServer.name}
        </Text>
        <Text color={t.color.muted}>Enter toggles the globally persisted runtime permission.</Text>
        <Box marginTop={1} flexDirection="column">
          {mcpTools.length ? (
            mcpTools.map((tool, row) => (
              <Text key={tool.name} color={row === mcpToolIndex ? t.color.accent : t.color.text} wrap="truncate-end">
                {row === mcpToolIndex ? '▸ ' : '  '}
                {tool.name}{' '}
                <Text color={tool.enabled ? t.color.muted : t.color.warn}>
                  [{tool.enabled ? 'enabled' : 'disabled'} · {tool.agentIds?.join(', ') || 'global'}]
                </Text>
              </Text>
            ))
          ) : (
            <Text color={t.color.muted}>no tools currently visible</Text>
          )}
        </Box>
        {notice ? <Text color={t.color.accent}>{notice}</Text> : null}
        {error ? <Text color={t.color.error}>error: {error}</Text> : null}
  ...
  if (view === 'detail' && selectedSkill) {
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>
          {selectedSkill.enabled ? 'enabled' : 'disabled'} · {selectedSkill.level} · {selectedSkill.source}
        </Text>
        {selectedSkill.whenToUse ? <Text color={t.color.muted}>when to use: {selectedSkill.whenToUse}</Text> : null}
        <Box marginTop={1}>
          <Text color={t.color.text} wrap="truncate-end">
            {selectedSkill.content || '(no skill body)'}
          </Text>
        </Box>
        {notice ? <Text color={t.color.accent}>{notice}</Text> : null}
        {error ? <Text color={t.color.error}>error: {error}</Text> : null}
  ...
  if (view === 'edit-mcp') {
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>
          Changes are saved to {configPath}; the Harness backend immediately reconciles the live connection.
        </Text>
        {fieldRows.map(([label, value], row) => (
          <Box key={label}>
            ...
            {field === row && row !== 1 ? (
              <TextInput columns={inputWidth} onChange={setters[row]!} value={values[row]!} />
            ) : (
              <Text color={t.color.text}>{value || '(empty)'}</Text>
            )}
          </Box>
        ))}
        {error ? <Text color={t.color.error}>error: {error}</Text> : null}
  ...
  return (
    <Box flexDirection="column" width={width}>
      ...
      <Text color={t.color.muted}>
        {tab === 'skills' ? '▸ Skills' : '  Skills'} {tab === 'mcp' ? '▸ MCP servers' : '  MCP servers'} · ←/→ switch
      </Text>
      {tab === 'skills' ? (
        skills.length ? (
          skills.map((skill, row) => (
            <Text color={row === clampedIndex ? t.color.accent : t.color.text} key={skill.path} wrap="truncate-end">
              {row === clampedIndex ? '▸ ' : '  '}
              {skill.name}{' '}
              <Text color={skill.enabled ? t.color.muted : t.color.warn}>
                [{skill.enabled ? 'enabled' : 'disabled'} · {skill.level}]
              </Text>
            </Text>
          ))
        ) : (
          <Text color={t.color.muted}>no managed Skills found</Text>
        )
      ) : servers.length ? (
        servers.map((server, row) => {
          ...
          const ownership =
            server.managed === false ? (server.runtimeDisabled ? ' · external runtime-disabled' : ' · externally managed') : ''
          const scopes = server.agentIds?.length ? ` · ${server.agentIds.join(', ')}` : ''
          return (
            <Text color={row === clampedIndex ? t.color.accent : t.color.text} key={server.name} wrap="truncate-end">
              {row === clampedIndex ? '▸ ' : '  '}
              {server.name}{' '}
              <Text color={color}>
      ...
      <OverlayHint t={t}>
        {tab === 'skills'
          ? '↑↓ select · Enter inspect · Space enable/disable · r refresh · Esc/q close'
          : '↑↓ select · Enter open menu · Space enable/disable · a add · r refresh status · Esc/q close'}
      </OverlayHint>
    </Box>
  )
}
