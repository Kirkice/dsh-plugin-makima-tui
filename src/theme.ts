...
// Mirrors @makima-tui/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @makima-tui/ink is bundled separately from packages/.
...
// Terminal. Explicit MAKIMA_TUI_THEME / COLORFGBG signals above still win,
...
// `MAKIMA_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `MAKIMA_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `MAKIMA_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `MAKIMA_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
export function detectLightMode(
  ...
  const lightFlag = (env.MAKIMA_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.MAKIMA_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.MAKIMA_TUI_BACKGROUND ?? '')
...
export function hasExplicitBackgroundSignal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (backgroundLuminance(env.MAKIMA_TUI_BACKGROUND ?? '') !== null) {
    return true
  }
