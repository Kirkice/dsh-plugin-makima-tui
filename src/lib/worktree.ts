 ...
 * and advertises it via MAKIMA_TUI_WORKTREE_* env vars (the same block the
 ...
 *
 * Ownership gate: the launcher stamps MAKIMA_TUI_WORKTREE_OWNER_PID with its
...
export function parseWorktreeSession(env: Record<string, string | undefined>, ppid: number): null | WorktreeSessionInfo {
  ...
  const originalCwd = env.MAKIMA_TUI_WORKTREE_ORIGINAL_CWD
  const repoRoot = env.MAKIMA_TUI_WORKTREE_REPO_ROOT

  if (!name || !path || !branch || !originalCwd || !repoRoot) {
    return null
  }

  const ownerPid = Number(env.MAKIMA_TUI_WORKTREE_OWNER_PID ?? '')
