...
//
// Both gate on MAKIMA_TUI_DEV_PERF=1 and dump JSON-lines (default ~/.makima-tui/perf.log,
// override MAKIMA_TUI_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// MAKIMA_TUI_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@makima-tui/ink'
import { scrollFastPathStats } from '@makima-tui/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.MAKIMA_TUI_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.MAKIMA_TUI_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.MAKIMA_TUI_DEV_PERF_LOG?.trim() || appHomePath('perf.log')
