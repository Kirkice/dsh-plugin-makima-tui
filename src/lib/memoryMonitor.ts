...
// Deferred @makima-tui/ink import: loading `@makima-tui/ink` at module top-level
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.MAKIMA_TUI_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@makima-tui/ink` off the cold-start critical path;
