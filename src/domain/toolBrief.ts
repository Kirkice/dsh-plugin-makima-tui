 ...
 *
 * STATUS: the transcript no longer folds runs. Every call keeps its own
 * `⏺ Tool(args)` row with a few lines of what it returned, because the path a
 * read opened and the command a shell ran are the transcript's whole point and
 * a tally took exactly those away. `isDelegationCall` (the tool vocabulary) is
 * what the renderer still uses from here; the clause and run helpers below have
 * no caller. They are kept, tested and intact, because reinstating the brief is
 * one line in ToolTrail — not because anything calls them today.
 */
