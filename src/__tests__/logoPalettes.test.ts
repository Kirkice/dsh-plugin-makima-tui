...
describe('readLogoColorSync', () => {
  const prevHome = process.env.MAKIMA_TUI_HOME
  let dir = ''

  afterEach(() => {
    if (prevHome === undefined) {
      delete process.env.MAKIMA_TUI_HOME
    } else {
      process.env.MAKIMA_TUI_HOME = prevHome
    }
  ...
  const home = (config?: string) => {
    dir = mkdtempSync(join(tmpdir(), 'makima-tui-logo-'))
    ...
    process.env.MAKIMA_TUI_HOME = dir
  }
