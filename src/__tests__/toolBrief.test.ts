...
describe('ToolTrail flat render', () => {
  ...
  it('names the expand key on a body its card held part of back', () => {
  ...
  it('offers no expand key when there is nothing more to show', () => {
    // a result the gateway kept no raw copy of expands to the same text, and
...
describe('estimatedMsgHeight matches the painted trail', () => {
  ...
  describe('through MessageLine', () => {
    ...
    it('does not charge the details margin to a structured diff', () => {
      ...
      // Turning details on costs the call's two rows (`⏺ Read(a.py)` over its
      // `⎿` result), plus the 2 the estimator charges for a `Response`
      // separator this branch never paints (older divergence, left alone).
      // The point is that it is 4 and not 5 — the details wrapper's
      // marginBottom belongs to prose rows only.
      expect(withDetails - withoutDetails).toBe(4)
    })
  })
  ...
  for (const cols of [60, 100]) {
    for (const [name, msg] of cases) {
      ...
      it(`agrees on ${name} (expanded, cols=${cols})`, () => {
        // ctrl+o expands BOTH — the flat per-call rows and the whole chain of
        // thought — so the estimate has to be handed both, exactly as
        // useMainApp derives them from the one global mode.
        expect(
        ...
        ).toBe(paintedRows(msg, 'expanded', cols))
      })
    }
  }
})
