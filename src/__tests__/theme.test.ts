...
// MAKIMA_TUI_THEME=light (or MAKIMA_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('makima tui')
    expect(DEFAULT_THEME.brand.prompt).toBe('❯')
    expect(DEFAULT_THEME.brand.tool).toBe('┊')
  })

  it('has the Makima JellyFish dark palette', async () => {
    const { DEFAULT_THEME } = await importThemeWithCleanEnv()

    expect(DEFAULT_THEME.color.primary).toBe('#E84393')
    expect(DEFAULT_THEME.color.accent).toBe('#7DD3E8')
    expect(DEFAULT_THEME.color.border).toBe('#B366FF')
    expect(DEFAULT_THEME.color.error).toBe('#FF3B6B')
    expect(DEFAULT_THEME.color.text).toBe('#EEEAF4')
    expect(DEFAULT_THEME.color.muted).toBe('#7E7888')
    expect(DEFAULT_THEME.color.ok).toBe('#00FF9C')
    expect(DEFAULT_THEME.color.warn).toBe('#FFD060')
    expect(DEFAULT_THEME.color.subtle).toBe('#655F70')
    expect(DEFAULT_THEME.color.planMode).toBe('#00E5FF')
    expect(DEFAULT_THEME.color.permission).toBe('#E84393')
  })
})
...
describe('detectLightMode', () => {
  ...
  it('honors MAKIMA_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ MAKIMA_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ MAKIMA_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ MAKIMA_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ MAKIMA_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ MAKIMA_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets MAKIMA_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', MAKIMA_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors MAKIMA_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ MAKIMA_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ MAKIMA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', MAKIMA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', MAKIMA_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses MAKIMA_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ MAKIMA_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // MAKIMA_TUI_THEME=light would flip the base palette and make these
...
describe('hasExplicitBackgroundSignal', () => {
  it('is true only for explicit env signals (NOT TERM_PROGRAM)', async () => {
    ...
    expect(hasExplicitBackgroundSignal({ MAKIMA_TUI_THEME: 'dark' })).toBe(true)
    expect(hasExplicitBackgroundSignal({ MAKIMA_TUI_THEME: 'light' })).toBe(true)
    expect(hasExplicitBackgroundSignal({ MAKIMA_TUI_LIGHT: '1' })).toBe(true)
    expect(hasExplicitBackgroundSignal({ MAKIMA_TUI_LIGHT: 'off' })).toBe(true)
    expect(hasExplicitBackgroundSignal({ MAKIMA_TUI_BACKGROUND: '#1a1a1a' })).toBe(true)
  ...
  it('mirrors detectLightMode: true exactly when it resolves before the TERM_PROGRAM fallback', async () => {
    ...
    const cases: Record<string, string>[] = [
      {},
      { MAKIMA_TUI_THEME: 'dark' },
      { MAKIMA_TUI_THEME: 'light' },
      { MAKIMA_TUI_LIGHT: '1' },
      { MAKIMA_TUI_LIGHT: '0' },
      { MAKIMA_TUI_BACKGROUND: '#ffffff' },
      { MAKIMA_TUI_BACKGROUND: '#000' },
      { COLORFGBG: '0;15' },
      { COLORFGBG: '0;7' },
      { COLORFGBG: '15;0' },
