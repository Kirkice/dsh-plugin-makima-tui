...
const fullEnv = (overrides: Record<string, string | undefined> = {}) => ({
  MAKIMA_TUI_WORKTREE_BRANCH: 'worktree-fix-auth',
  MAKIMA_TUI_WORKTREE_NAME: 'fix-auth',
  MAKIMA_TUI_WORKTREE_ORIGINAL_CWD: '/repo',
  MAKIMA_TUI_WORKTREE_OWNER_PID: String(OWNER_PID),
  MAKIMA_TUI_WORKTREE_PATH: '/repo/.clawcodex/worktrees/fix-auth',
  MAKIMA_TUI_WORKTREE_REPO_ROOT: '/repo',
  ...overrides
})
...
describe('parseWorktreeSession', () => {
  ...
  it('returns null when any var is missing', () => {
    for (const key of [
      'MAKIMA_TUI_WORKTREE_NAME',
      'MAKIMA_TUI_WORKTREE_PATH',
      'MAKIMA_TUI_WORKTREE_BRANCH',
      'MAKIMA_TUI_WORKTREE_ORIGINAL_CWD',
      'MAKIMA_TUI_WORKTREE_REPO_ROOT'
    ]) {
  ...
  it('rejects a stale/leaked env block via the OWNER_PID ppid gate', () => {
    ...
    expect(parseWorktreeSession(fullEnv({ MAKIMA_TUI_WORKTREE_OWNER_PID: undefined }), OWNER_PID)).toBeNull()
    expect(parseWorktreeSession(fullEnv({ MAKIMA_TUI_WORKTREE_OWNER_PID: 'nope' }), OWNER_PID)).toBeNull()
    expect(parseWorktreeSession(fullEnv({ MAKIMA_TUI_WORKTREE_OWNER_PID: '0' }), 0)).toBeNull()
  })
})
