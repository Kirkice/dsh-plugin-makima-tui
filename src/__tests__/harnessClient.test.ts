...
const TOOL_CARDS: Record<string, unknown> = {
  ...
  read_long: {
    card: 'read',
    lines: Array.from({ length: 10 }, (_, i) => ({ number: i + 1, text: `l${i + 1}` })),
    offset: 1,
    path: 'src/big.ts',
    totalLines: 10
  },
...
describe('HarnessGatewayClient', () => {
  ...
  it('shows the head of the file it read, not a tally', async () => {
    const done = await runTool('read_window', '{"file_path":"src/y.ts","offset":8}')

    // a tally says a read happened without saying what it found; the row exists
    // so a reader can follow along without opening the file themselves
    expect(done.payload.result_text).toBe(' 8  import x\n 9  \n10  export const y = 1')
    expect(done.payload.result_raw).toBe(' 8  import x\n 9  \n10  export const y = 1')
  })

  it('holds back a read longer than the row, and says how much', async () => {
    const done = await runTool('read_long', '{"file_path":"src/big.ts"}')

    expect(done.payload.result_text).toBe(' 1  l1\n 2  l2\n 3  l3\n 4  l4\n… +6 lines')
    expect(done.payload.result_raw?.split('\n')).toHaveLength(10)
  })
