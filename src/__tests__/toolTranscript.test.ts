...
// @makima-tui/ink import (PassThrough stdout has isTTY=false otherwise).
vi.hoisted(() => {
...
describe('estimatedMsgHeight with multi-line tool details', () => {
  ...
  it('counts the same detail rows collapsed, since the call keeps its block', () => {
    // Every call renders its own `⏺ …` + `⎿ …` block in both views; ctrl+o
    // changes how much of the RESULT shows (the verbose sibling), not whether
    // the call does. The estimate has to agree or the scrollbar and topSpacer
    // math drift against the paint.
    expect(estimatedMsgHeight(base, 80, opts) - estimatedMsgHeight(single, 80, opts)).toBe(2)
  })
})
...
describe('ToolTrail rendering', () => {
  ...
  it('marks failed tools with an error bullet while the name row stays plain', () => {
    ...
    // error red on the bullet: JellyFish error #FF3B6B
    expect(output).toContain('255;59;107')
  })
})
