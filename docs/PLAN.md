# Makima TUI — Implementation Plan
...
| Project | Role |
|---|---|
| `reference_projects/clawcodex/ui-tui` | The TUI to port: ~41k LOC React app + ~29k LOC forked Ink renderer (`packages/makima-tui-ink`). MIT. |
...
- Root `package.json` (`makima-tui`, ESM, React 19, `file:` dep on the fork), `tsconfig.json`,
  `vitest.config.ts`, CI workflow (typecheck + vitest).
- Vendor `packages/makima-tui-ink` unchanged (esbuild build, root `index.js` shims).
...
- `/model` via `ctx.llm` catalog + `installModelSelection` + `ctx.agentDefaultModel`; `/compact`
  via `ctx.compaction`; `/status`, `/help`, `/usage`; `config.get/set` over a `makima-tui`
  settings namespace.
...
- `bin/makima-tui.js` launcher (profile bootstrap via `dsh plugin add`, version-skew guard,
...
- 2026-08-18: v0.2.1 — ocean-blue brand ramp + DeepSeek-blue theme hue; the app owns
  `~/.makima-tui` (no longer reads clawcodex's config); `/logo` persistence implemented;
