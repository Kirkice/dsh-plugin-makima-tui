# Makima TUI Architecture

```
┌────────────────────────────────────────────────────────────────────┐
│ dsh (cordis runtime, profile "cc")                                 │
│  @deepseek-ai/dsh-base layer                                       │
│  + makima-tui cordis.patch.yml layer                               │
│    └─ plugin row: makima-tui                                       │
...
│        packages/makima-tui-ink  (forked Ink renderer, vendored)     │
└────────────────────────────────────────────────────────────────────┘
```
...
| `config.get` / `config.set` | `ctx.settings` namespace `makima-tui` (display prefs also mirrored in `~/.makima-tui/`) |
...
- **Sessions are harness sessions**: JSONL persistence, projections, and resume come from the
  harness; the TUI's own `~/.clawcodex` config/history files move to `~/.makima-tui/`.
...
├── bin/makima-tui.js       # launcher: profile bootstrap + skew guard (Stage 9)
├── packages/makima-tui-ink/ # vendored fork, unchanged (file: dependency)
├── src/
