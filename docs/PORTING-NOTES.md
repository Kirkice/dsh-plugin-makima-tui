...
- Distribution is checkout-based: `./install.sh` builds locally then
  `dsh plugin --profile makima-tui add <checkout>` (a pnpm link install). The bundle's
  cordis.patch.yml supplies the makima-tui row; module resolution from the linked checkout finds
  the repo's own node_modules, so the plugin externals resolve without touching the profile.
- `bin/makima-tui.js` is a thin launcher (probe dsh, verify the profile, exec
  `dsh --profile makima-tui` with NODE_ENV=production).
...
- Wordmark: DSH-MAKIMA_TUI in the same ANSI-shadow style (69 cols, hand-assembled, uniform width).
...
- brand: name `makima tui`, icon 🐳; tagline "Claude Code style TUI for Deepseek-Harness"
  (mid tier: "Claude Code style TUI"; tiny tier: "makima tui").
- Every user-visible "clawcodex" string was renamed; functional identifiers stay
  (CLAWCODEX_* env vars, ~/.clawcodex data dir, @makima-tui/ink package name).
- Identifier rename: every `cc-tui` token became `makima-tui` (package `makima-tui`, row id
  `makima-tui`, bin `makima-tui`, env prefix `MAKIMA_TUI_*`); the default profile is `makima-tui`.
- SessionInfo.version now carries the real plugin version (package.json, resolved from
  dist/ or src/), so the panel reads "makima tui v0.2.0".
...
- The banner shipped GREEN despite the v0.2.0 rebrand: `readLogoColorSync` read
  ...
  default. Two fixes: the app now owns `~/.makima-tui` (`src/lib/appHome.ts`, env override
  `MAKIMA_TUI_HOME`) for the logo pref, prompt history, lifecycle logs, perf log, heapdumps
...
- `packages/clawcodex-ink` → `packages/makima-tui-ink`, package `@clawcodex/ink` →
  `@makima-tui/ink` (143 files, ~29k LOC; 96 import sites, three esbuild aliases, the ambient
...
- Environment knobs `CLAWCODEX_*` / `CLAWCODEX_TUI_*` → `MAKIMA_TUI_*` (49 names; the
  redundant `TUI_` infix was dropped, e.g. `CLAWCODEX_TUI_INLINE` → `MAKIMA_TUI_INLINE`).
...
- The inert `/memory` feature's file name became `DSH-MAKIMA_TUI.md` under `~/.makima-tui`.
